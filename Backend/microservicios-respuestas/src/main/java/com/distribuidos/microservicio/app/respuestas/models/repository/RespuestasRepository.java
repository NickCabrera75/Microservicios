package com.distribuidos.microservicio.app.respuestas.models.repository;
...
public interface RespuestasRepository extends CrudRepository<Respuesta, Long> {
	...
	@Query("select r from Respuesta r join fetch r.alumno a join fetch r.pregunta p "
			+ "join fetch p.examen e where a.id =?1 and e.id=?2")
	public Iterable<Respuesta> findRespuestaByAlumnoByExamen(Long alumnoId, Long examenId);
	
	@Query("select e.id from Respuesta r join r.alumno a join r.pregunta p "
